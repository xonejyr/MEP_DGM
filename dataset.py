...
def extract_xy(pts):
    """
    从 pts 中提取 (x, y) 坐标，兼容多种形状:
    - [num_joints, 2]
    - [num_joints, 2, 2]
    - [num_joints, 3] (含 valid)
    """
    pts = np.array(pts)

    if pts.ndim == 2 and pts.shape[1] >= 2:
        # [num_joints, 2] 或 [num_joints, 3]
        return pts[:, :2]

    elif pts.ndim == 3 and pts.shape[1] >= 2:
        # [num_joints, 2, 2] → 取前两列作为 (x,y)
        return pts[:, 0, :2]

    else:
        raise ValueError(f"无法解析坐标形状: {pts.shape}")
...
def visualize_dataset_joints_distribution(dataset, save_dir="joints_dist_vis"):
    """
    可视化 dataset 的 joints 分布图（使用基于 GMM 的参数估计热图表示分布）：
    1. 为每个关节生成单独的 GMM 热图，中心为训练集均值，GMM 组件数由指定列表决定。
    2. 生成一张综合图，叠加所有关节的 GMM 热图（使用不同颜色区分），并标记每个关节的平均位置。
    所有图的输出分辨率均为 2400 高 x 1935 宽。
    """
    os.makedirs(save_dir, exist_ok=True)

    num_joints = dataset.num_joints
    H, W = 2400, 1935
    colors = cm.get_cmap("hsv", num_joints)  # 不同关节用不同颜色
    #colors = sns.color_palette("husl", num_joints)  # 替换 HSV，生成 num_joints 个均匀颜色

    # 指定每个关节的 GMM 组件数
    n_components_list = [2, 2, 1, 2, 1, 2, 2, 2, 1, 1, 1, 1, 1, 2, 2, 1, 2, 1, 1]
    if len(n_components_list) != num_joints:
        raise ValueError(f"GMM 组件数列表长度 ({len(n_components_list)}) 与 num_joints ({num_joints}) 不匹配")

    # 收集所有样本的 joints 数据
    all_joints = [[] for _ in range(num_joints)]
    for idx in range(len(dataset)):
        pts = dataset.load_annotation(idx)
        # Handle potential extra dimension in annotations
        if pts.ndim == 3 and pts.shape[-1] == 2:
            joints = pts[..., 0]  # Extract first set of coordinates
        elif pts.shape[-1] >= 2:
            joints = pts[:, :2]
        else:
            joints = pts
        ...
        for j in range(num_joints):
            all_joints[j].append(joints[j])

        if idx % 100 == 0:
            print(f"[{idx}/{len(dataset)}] joints 已收集")

    # 创建网格用于 GMM 分布计算
    x = np.linspace(0, W, 800)
    y = np.linspace(0, H, 800)
    X, Y = np.meshgrid(x, y)
    pos = np.dstack((X, Y))
    ...
    hists = []
    for j in range(num_joints):
        points = np.array(all_joints[j])
        # 确保 points 为 (num_samples, 2)
        if points.ndim != 2 or points.shape[1] != 2:
            raise ValueError(f"关节 {j} 的 points 形状异常: {points.shape}")
        ...
        for i in range(n_comp):
            comp_mean = gmm.means_[i]
            comp_cov = gmm.covariances_[i]
            comp_weight = gmm.weights_[i]
            # 防止协方差矩阵不可逆
            if np.linalg.matrix_rank(comp_cov) < 2:
                comp_cov += np.eye(2) * 1e-6
            try:
                gauss = multivariate_normal(mean=comp_mean, cov=comp_cov)
                pdf += comp_weight * gauss.pdf(pos.reshape(-1, 2)).reshape(pos.shape[:2])
            except np.linalg.LinAlgError:
                print(f"警告: 关节 {j} 组件 {i} 的协方差矩阵不可逆，跳过该组件")
    ...
    for j in range(num_joints):
        ...
        plt.imshow(hists[j], origin='lower', extent=[0, W, 0, H], cmap='hot', aspect='auto')
        ...
        out_path = os.path.join(save_dir, f"joint_{j:02d}_distribution.png")
        plt.savefig(out_path, dpi=100, bbox_inches='tight', pad_inches=0)
        plt.close()
        print(f"✅ 关节 {j} 分布图保存: {out_path}")

    # 2) 生成综合分布图（叠加所有关节的热图，使用不同颜色）
    plt.figure(figsize=(W / 100, H / 100))
    
    # 创建 RGB 复合图像
    ny, nx = hists[0].shape
    combined_hist = np.zeros((ny, nx, 3))
    for j in range(num_joints):
        norm_hist = hists[j] / np.max(hists[j]) if np.max(hists[j]) > 0 else hists[j]
        color = colors(j)[:3]
        combined_hist += norm_hist[..., np.newaxis] * color
    
    # 裁剪到 [0, 1]
    combined_hist = np.clip(combined_hist, 0, 1)
    
    plt.imshow(combined_hist, origin='lower', extent=[0, W, 0, H], aspect='auto')
    
    # 标记所有平均位置（使用对应颜色）
    for j in range(num_joints):
        x, y = means[j]
        plt.scatter(x, y, color=colors(j)[:3], s=50, marker='x')
    
    plt.axis('off')
    plt.gca().invert_yaxis()
    out_path = os.path.join(save_dir, "all_joints_distribution.png")
    plt.savefig(out_path, dpi=100, bbox_inches='tight', pad_inches=0)
    plt.close()
    print(f"✅ 综合 joints 分布图保存: {out_path}")

def visualize_dataset_joints(dataset, save_dir="joints_vis"):
    """
    可视化 dataset 的 joints:
    1. 绘制单张图的 joints，验证坐标和图像对应。
    2. 将所有样本的 joints 分布绘制到一张大图上。
    """
    os.makedirs(save_dir, exist_ok=True)
    ...
    ### -------------------------------
    ### 1) 验证单张图像 + joints 对应关系
    ### -------------------------------
    ##idx = 0  # 随便取一个样本
    ##image = dataset.load_image(idx)  # 原始图像 [2400, 1935, 3]
    ##pts = dataset.load_annotation(idx)  # [num_joints, 2, ...]
    ##joints = extract_xy(pts)
##
    ### 在图像上绘制关节
    ##vis_img = image.copy()
    ##for j in range(num_joints):
    ##    x, y = map(int, joints[j])
    ##    cv2.circle(vis_img, (x, y), 8, tuple(int(c*255) for c in colors(j)[:3]), -1)
##
    ##out_path = os.path.join(save_dir, f"check_sample_{idx:05d}.png")
    ##cv2.imwrite(out_path, vis_img)
    ##print(f"✅ 校验图像保存: {out_path}")

    # -------------------------------
    # 2) 绘制所有样本的 joints 分布到一张大图
    # -------------------------------
    H, W = 2400, 1935
    plt.figure(figsize=(12, 15))
    plt.imshow(np.zeros((H, W)), cmap="gray")  # 黑底背景
    ...
    for idx in range(len(dataset)):
        ...
        for j in range(num_joints):
            x, y = joints[j]
            plt.scatter(x, y, s=5, color=colors(j), alpha=0.5)

        if idx % 100 == 0:
            print(f"[{idx}/{len(dataset)}] joints 已绘制到大图")

        

    plt.axis("equal")
    plt.axis("off")
    #plt.gca().invert_yaxis()  # y 轴方向和图像一致
    out_path = os.path.join(save_dir, "all_joints_distribution.png")
    plt.savefig(out_path, dpi=300, bbox_inches="tight", pad_inches=0)
    plt.close()
    print(f"✅ joints 分布图保存: {out_path}")
